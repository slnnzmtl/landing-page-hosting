...
-- Ensure Data API roles can read these tables (re-run this file after changes)
grant select on table public.category to anon, authenticated;
grant select on table public.expense to anon, authenticated;
grant update on table public.expense to anon, authenticated;
...
create policy "anon_update_expense"
  on public.expense
  for update
  to anon, authenticated
  using (true)
